...
**触发关键词**：`生成测试模块`、`生成功能模块`、`识别测试模块`、`创建模块`

**工作流简介**：基于需求分析和参考节点，识别并生成功能测试模块（FEATURE类型节点），梳理需要测试的功能模块划分
...
- **检索目标**：从需求分析结果中提取关键词进行检索
  - **关键词提取方法**：
    - 从步骤3的需求分析结果中识别核心功能模块（如"用户注册"、"表单验证"）
    - 提取关键业务术语和专业词汇（如"鉴权"、"校验规则"）
    - 识别技术要点（如"API接口"、"数据库字段"）
    - 建议提取 3-5 个最具代表性的关键词
  - **检索策略**：
    - **优先策略**：使用 2-3 个核心关键词组合检索（如"用户注册 表单验证"），获取高相关性内容
...
**步骤 5：生成测试模块**
- **使用工具**：`select_rule`（位于 `tools/select_rule`），传入关键词"功能生成"
- **根据返回结果**：
  - 如果返回自定义规则路径（`.codebuddy/rules/xxx`）：使用 `read_rules` 读取规则内容
  - 如果返回默认Generator路径（`generators/xxx`）：使用 `read_file` 读取 Generator 内容
- **执行方式**：按获取到的规则/Generator定义执行模块生成
- **标准输入参数**：
  ```python
  {
    "ref_node": {  # 参考节点信息
    ...
    "knowledge_context": "{步骤5检索到的历史模块划分参考（如有）}"
  }
  ```
- **输出要求**：
  - ✅ **必须输出标准JSON数组格式**（参考 `generators/feature-generator.md`）
  - ✅ 每个模块节点必须包含完整字段：`uid`, `name`, `description`, `kind`, `parent_uid`, `instance`
