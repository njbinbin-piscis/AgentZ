...
This skill provides comprehensive guidance for discovering, recommending, installing, and comparing plugins from AgentZ Code and AgentZ plugin marketplaces. Enable intelligent plugin discovery through AI semantic matching, streamlined installation workflows, and multi-plugin comparison capabilities.
...
AgentZ Code supports both AgentZ and Claude plugin ecosystems:

**AgentZ Platform:**
- Marketplace registry: `~/.AgentZ/plugins/known_marketplaces.json`
- Plugin installation uses AgentZ-specific paths
- Uses `.AgentZ-plugin/` directory structure
...
```json
{
  "claude-plugins-official": {
    ...
    "installLocation": "/Users/user/.AgentZ/plugins/marketplaces/...",
    "manifest": {
      "plugins": [...]
    }
  }
...
```bash
# AgentZ marketplaces
cat ~/.AgentZ/plugins/known_marketplaces.json
...
Use `chat_ui tool` tool to display recommendations with:
...
⚠️  IMPORTANT: You must restart AgentZ Code for plugins to take effect.

Steps:
1. Exit AgentZ Code completely
2. Relaunch AgentZ Code
...
```bash
ls ~/.AgentZ/plugins/cache/marketplace-name/plugin-name/
```
...
Users can configure plugin finder behavior via `~/.AgentZ/.local.md`:
...
**Solutions:**
1. **Primary**: Restart AgentZ Code (exit completely and relaunch)
2. Verify installation: `ls ~/.AgentZ/plugins/cache/`
3. Check plugin.json syntax
4. Review AgentZ Code logs
...
1. **Read both platform registries**: Check both `~/.AgentZ/` and `~/.claude/` paths
