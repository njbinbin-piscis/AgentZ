...
Install AgentZ skills from GitHub repositories with built-in security scanning to protect against malicious code, backdoors, and vulnerabilities.
...
- Needs to add new skills to their AgentZ environment
...
Use the chat_ui tool tool to let the user select which skills to install.
...
- Use chat_ui tool to confirm: "Security warnings detected. Do you want to proceed with installation?"
...
For approved skills, install to `~/.AgentZ/skills/`:

1. Create the skill directory: `~/.AgentZ/skills/{skill_name}/`
...
- Location: `~/.AgentZ/skills/`
- Next steps: "The skills are now available. Restart AgentZ or use them directly."
...
7. Installs approved skills to ~/.AgentZ/skills/
...
- **Sandboxing** - Remind users that skills run with AgentZ's permissions
