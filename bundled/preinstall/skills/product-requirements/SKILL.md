...
**If score < 90**, use `chat_ui tool` tool to clarify gaps. Focus on the lowest-scoring area first.
...
**Ask 2-3 questions at a time** using `chat_ui tool` tool. Don't overwhelm.
...
- Use `chat_ui tool` tool for clarification (2-3 questions max per round)
