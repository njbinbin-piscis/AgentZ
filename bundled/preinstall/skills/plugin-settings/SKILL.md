...
# Plugin Settings Pattern for AgentZ Code Plugins

## Overview

Plugins can store user-configurable settings and state in `.AgentZ/plugin-name.local.md` files within the project directory. This pattern uses YAML frontmatter for structured configuration and markdown content for prompts or additional context.

**Key characteristics:**
- File location: `.AgentZ/plugin-name.local.md` in project root
...
- Prompts to feed back to AgentZ
- Documentation or notes
```

### Example: Plugin State File

**.AgentZ/my-plugin.local.md:**
```markdown
---
enabled: true
strict_mode: false
...
STATE_FILE=".AgentZ/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.AgentZ/my-plugin.local.md`
...
Check for plugin settings at `.AgentZ/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".AgentZ/security-scan.local.md"
...
**.AgentZ/multi-agent-swarm.local.md:**
```markdown
---
...
2. Create `.AgentZ/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart AgentZ Code for hooks to recognize changes
```
...
Create `.AgentZ/my-plugin.local.md` in your project:
...
After creating or editing, restart AgentZ Code for changes to take effect.
```
...
✅ **DO:**
- Use `.AgentZ/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.AgentZ/`)
...
**Important:** Settings changes require AgentZ Code restart.
...
After editing `.AgentZ/my-plugin.local.md`:
1. Save the file
2. Exit AgentZ Code
...
```bash
if [[ ! -f ".AgentZ/my-plugin.local.md" ]]; then
...
3. Add gitignore entry for `.AgentZ/*.local.md`
...
7. Remind users that changes require AgentZ Code restart
